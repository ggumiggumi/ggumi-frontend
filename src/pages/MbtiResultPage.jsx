    ...
    const fetchChildMbti = async () => {
      try {
        ...
        const response = await fetch(
          `${API_DOMAIN}/histories/my-child-mbti?childId=${childId}`,
          {
            method: "GET",
            headers: {
  ...
  const deleteRequest = async () => {
    ...
    if (isConfirmed) {
      try {
        const childId = sessionStorage.getItem("childId");
        const response = await fetch(
          `${API_DOMAIN}/children/${childId}/soft-delete`,
          {
            method: "DELETE",
            headers: {
              "Content-Type": "application/json",
        ...
        alert("진단 데이터가 성공적으로 삭제되었습니다.");
        navigate("/main"); // 삭제 후 메인 페이지로 이동
      } catch (error) {
        console.error("진단 데이터 삭제 중 에러 발생:", error);
        alert("진단 데이터 삭제에 실패했습니다. 다시 시도해 주세요.");
      }
    }
  };
              ...
              <div className="mbti-info">
                ...
                <div>
                  {mbtiTags.map((tag, index) => (
                    <div className="mbti-tag" key={index}>
                      {tag}
                    </div>
                  ))}
                </div>
                ...
                <div key={index}>
                  ...
                  <div className="bar-container">
                    <div
                      ...
                      style={{
                        width: `${
                          mbtiValues[index][Object.keys(mbtiValues[index])[0]]
                        }%`,
                        backgroundColor:
                          mbtiColors[index][Object.keys(mbtiValues[index])[0]], // 해당 색상 적용
                      }}
                    >
                    ...
                    <div
                      ...
                      style={{
                        width: `${
                          mbtiValues[index][Object.keys(mbtiValues[index])[1]]
                        }%`,
                        backgroundColor:
                          mbtiColors[index][Object.keys(mbtiValues[index])[1]], // 해당 색상 적용
                      }}
                    >
                      <span className="percentage">
                        {mbtiValues[index][Object.keys(mbtiValues[index])[1]]}%
